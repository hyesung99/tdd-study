...
import { sortByDate, sortByLikes, sortByComments, sortQuestions } from './sorting';
...
describe('sortByDate', () => {
  it('최신순으로 정렬 (내림차순)', () => {
    ...
    expect(result[0].id).toBe('4'); // 2024-01-04
    expect(result[1].id).toBe('2'); // 2024-01-03
    expect(result[2].id).toBe('3'); // 2024-01-02
    expect(result[3].id).toBe('1'); // 2024-01-01
  });
...
describe('sortByComments', () => {
  ...
  it('댓글 수가 같으면 최신순으로 정렬', () => {
    const questionsWithSameComments: Question[] = [
      { ...mockQuestions[0], commentCount: 5, createdAt: new Date('2024-01-01') },
      { ...mockQuestions[1], commentCount: 5, createdAt: new Date('2024-01-03') },
      { ...mockQuestions[2], commentCount: 5, createdAt: new Date('2024-01-02') },
    ];
